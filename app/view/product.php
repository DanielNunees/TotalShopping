...
<body>
  ...
  <div class="thumbnail container "  >
    <?php if(isset($products[0]['image'])){ ?>
      <img class="center-block" id="pic" src="<?php echo 'http://127.0.1.1/prestashop'.$products[0]['image'];?>" width="300" height="300" >
      <?php }else{?>
          <img class="center-block" src="http://127.0.1.1/prestashop/img/p/br-default-thickbox_default.jpg" width="300" height="300">
      <?php } ?>
      <div class="caption" id="extra_pics" >
        <?php if(isset($final)) { ?>
          <?php $q=0; foreach ($final[$q]['photos'] as $img) { ?>
            <img onclick="zoom_min_pics('<?php echo $final[0][0] ?>',<?php echo $q ?>)" class="img-thumbnail left-block" id="min_pics" src="<?php echo 'http://127.0.1.1/prestashop'.$img?>" width="93" height="93">
         <?php $q++; }?>
        <?php } ?> 
      </div>
  </div>
      ...
      <?php $i=0; foreach($final as $end){ ?>
          if (id.localeCompare("<?php echo $end[0];?>")==0) {
            <?php if(isset($end['photos'][0])){ ?>
              ...
              document.getElementById("pic").src = "<?php echo 'http://127.0.1.1/prestashop'.$end['photos'][0]; ?>";
            <?php }?>
